...
Read `OPENBOT.md` and `brains/STATUS.md` before changing anything.
This repo is a control plane for **OttoBot**. Do not invent a third agent runtime.
Also follow `.cursor/rules/ceo-index-build.mdc` for CEO / desk status / Accept work.
...
3. End jobs by updating `brains/STATUS.md` or the agent brain: Now / Last / Next / Blocker.
4. No secrets in files. No passwords, TOTP, or API keys in git, brains, or chat logs.
5. New tools default OFF for Cos.
6. If you stall, write one-line Blocker in desk status and stop.
...
If Builder folder → change → diff card → desk status is not delightful, stop adding features.
