...
| Engines chip | Header chip, red if missing, never fake green | First walk: chip lived in footer and was easy to miss | Moved `#enginesChip` into header `.bar-right`; click opens Settings → Engines | After restart + cache v=169 |
...
| Run Think now | Proposal + Do it / Ask Cos / Ask me from Chat | Hermes: not logged into Nous Portal | Honest wall. Cron stays off. INDEX Blocker names Nous login | Wall logged; no cron attached |
| Receipts | tokens / $ | Job JSON had tokens; UI hid them | `receiptLine` shows `{prompt}→{output} tok · $` (`$0.0000` ok) | Marker test |
| OpenCode / Hermes panes | Real session or wall, not blank fake | OpenCode web threw on `/data`; iframe proxy 502 | JSON error + `.embed-aim.wall`; hide iframe; no fake src | After restart |
...
| Secrets | None in git / QA / chat | Pass | — | Pass |
