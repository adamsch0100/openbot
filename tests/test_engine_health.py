...
class EngineHealthTests(unittest.TestCase):
    ...
    def test_gateway_down_next_step(self):
        ...
        with patch.object(eh, "detect", return_value={
            ...
            with patch.object(eh, "_run_version", return_value="0.21.1"):
                with patch.object(eh, "hermes_dash_status", return_value={"running": True, "home": "/data/hermes-homes/listlogic"}):
                    with patch.object(eh, "opencode_web_status", return_value={"running": False, "folder": ""}):
                        with patch.object(eh, "_port_open", return_value=True):
                            with patch.object(eh, "_live_dash_home", return_value="/data/hermes-homes/listlogic"):
                                with patch.object(eh, "resolve_ceo_hermes_home", return_value="/data/hermes-homes/listlogic"):
                                    with patch("openbot.org.project_tools", return_value={"hermes_home": "/data/hermes-homes/listlogic"}):
                                        with patch("openbot.hermes.gateway_status", return_value={"running": False, "ok": False}):
                                            row = eh.engine_health("listlogic")
    ...
    def test_saa_imported_off_is_not_restart(self):
        from openbot import engine_health as eh
        ...
        self.assertIsNone(row.get("action"))
        self.assertTrue(any("Live SAA" in n for n in row["next"]))
        self.assertFalse(any("Restart" in n for n in row["next"]))

    def test_saa_desk_owns_gateway_off_is_restart(self):
        from openbot import engine_health as eh
    ...
    def test_dash_mismatch_with_gateway_up_is_not_restart(self):
        from openbot import engine_health as eh
        ...
        with patch.object(eh, "detect", return_value={
            ...
            with patch.object(eh, "_run_version", return_value="0.21.1"):
                with patch.object(eh, "hermes_dash_status", return_value={"running": True, "home": "/root/.hermes"}):
                    with patch.object(eh, "opencode_web_status", return_value={"running": True, "folder": "/app"}):
                        with patch.object(eh, "_port_open", return_value=True):
                            with patch.object(eh, "_live_dash_home", return_value="/root/.hermes"):
                                with patch.object(eh, "resolve_ceo_hermes_home", return_value="/data/hermes-homes/openbot"):
                                    with patch("openbot.org.project_tools", return_value={"hermes_home": "/data/hermes-homes/openbot"}):
                                        with patch("openbot.hermes.gateway_status", return_value={"running": True, "ok": True}):
                                            row = eh.engine_health("openbot")
        self.assertNotEqual(row.get("action"), "restart_gateway")
        self.assertTrue(row["hermes"]["gateway_running"])
        self.assertFalse(row["hermes"]["dash_home_ok"])
        self.assertTrue(any("Tools" in n for n in row.get("next") or []))
        self.assertFalse(row["ok"])

    def test_saa_imported_dash_mismatch_is_not_red(self):
        from openbot import engine_health as eh
        ...
        self.assertTrue(any("Live SAA" in n for n in row.get("next") or []))
        self.assertFalse(any("Restart" in n and "imported" not in n for n in row.get("next") or []))
